...
- **Style switcher hidden in preview** (`c71dda1`) — `groupMembers` query was `enabled: … && !isPreview` and the `openPreview` payload lacked `product_group_id`. Fix: payload now carries `product_group_id` + `styleLabel`, and the query runs in preview (clicking a sibling navigates to its real page).
- **CMS "Edit Page" panel stale / empty** (`183b893`, `SiteContentContext.tsx`) — the provider called `fetchAll` once on mount and never refetched, so rows were effectively per-tab and frozen; edits in the admin dashboard tab never reached an already-open public tab's Edit Page panel (or the reverse). Root cause isolated with DB evidence: `site_content` is public-read (`SELECT USING (true)`, so no auth-visibility race) and has **no duplicate `(page,section,key)` rows**; within one tab both editors share the same provider + `FieldInput`, so in-tab divergence is impossible → it had to be cross-tab staleness. Fix: refetch on `window` `focus` + `visibilitychange`, and keep existing rows on a failed fetch instead of blanking. Verified: mutating a value in the DB + firing `focus` updated the live page with no reload; live admin session showed populated fields on About + Home (only blanks were optional `q_*` quote slots). Note: the SiteEditor "Edit Page" panel only renders on the CMS pages in `PATH_TO_PAGE` (home/shop/about/contact/faq/advertise); product/blog pages show an "Edit Product"/"Edit Post" link instead.
- **Style-switcher polish** (`4f016ca`, `ProductDetail.tsx`) — render current + siblings as one **name-sorted** list with the current one highlighted (previously the current product was always rendered first, so switching floated it to the front — the "rotating"). Bigger buttons (`px-6 py-3 text-sm`, `gap-2.5`). Fallback `shortLabel` strips a trailing "Silhouette" so derived labels collapse to one word; admin `style_label` overrides. One-word `style_label` values set on all 9 grouped products (Male/Female for finger, pourn-hand, 2nd-most-fun-job silhouettes; Graphic/Text for atheist + the text-only variants).
