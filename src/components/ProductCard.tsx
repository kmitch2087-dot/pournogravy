    ...
    >
      <Link to={`/product/${product.id}`} className="group block">
        <div className="relative aspect-square bg-muted overflow-hidden border-2 border-foreground/10 rough-border">
          {cardImage ? (
            <img src={cardImage} alt={product.name} className="absolute inset-0 w-full h-full object-cover scale-[1.35] group-hover:scale-[1.45] transition-transform duration-500" />
          ) : (
