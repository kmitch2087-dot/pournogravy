...
export default function Analytics() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {/* AOV Trend */}
      {chartData.length > 0 && (
        <SectionCard title="Average Order Value — Last 30 Days">
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart
              data={chartData.filter((d) => d.orders > 0).map((d) => ({
                date: d.date,
                aov: parseFloat((d.revenue / d.orders).toFixed(2)),
              }))}
            ...
            >
              ...
              <Tooltip content={<CustomTooltip />} />
              <Area type="monotone" dataKey="aov" stroke="#34d399" strokeWidth={2} fill="url(#aovGrad)" />
            </AreaChart>
          </ResponsiveContainer>
        </SectionCard>
      )}
