...
|--------|--------------|
| (branch `feat/finances-stripe-grade-reporting`, Jul 31–Aug 1) | **Finances — Stripe-grade reporting suite + gated sales-tax collection.** `/admin/finances` reworked from a half-wired 6-tab page into a full **7-tab IA** (Overview · Reports · Payouts · Expenses · Products · Invoices · Tax Packet, `Finances.tsx`, `?tab=`); all previously-orphaned pages (`BookkeepingReports`, `BookkeepingProducts`, `BookkeepingExpenses`, `BookkeepingTaxPacket`, new `Payouts.tsx`) wired in. **One report engine** — `generate-report` extended to emit `format:"json"` (on-screen tables + charts) alongside existing `csv`/`html`, and grew from 5 to **10 report types**: P&L, Order Summary, Sales by Product, Expense Detail, Stripe Fee Summary, plus new **Sales Tax Collected**, **Top Customers/LTV**, **Refunds & Disputes**, **Payout Reconciliation** (Stripe payouts + balance-transaction reconciliation, on-demand, no new table), and **Tax Estimate** (federal/SE estimator moved off Overview into a report). Overview COGS bug fixed — was a flat `$12/item` constant, now reads real per-product `products.cost_cents`. **Sales tax collection** (OFF by default, `settings.tax_enabled=false`): since checkout uses a custom Stripe PaymentIntent (not Checkout Sessions), `automatic_tax` doesn't apply — built a server round-trip instead: new `calculate-tax` edge fn calls the Stripe **Tax Calculation API**, updates the PI amount before confirm; `stripe-webhook` (v63) records a Tax Transaction on paid orders; `refund-order` (v7) reverses it on refund; `close-month` (v5) rolls up `tax_collected_cents`. New columns: `orders.tax_cents`, `orders.stripe_tax_calculation_id`, `orders.stripe_tax_transaction_id`, `monthly_snapshots.tax_collected_cents`, `settings.tax_enabled`. ⚠️ **Not yet enabled on live checkout** — needs a Stripe test-mode end-to-end pass and apparel `tax_code`s set on products first (clothing exemptions won't apply under the account's default tax code). Full task ledger: `.superpowers/sdd/progress.md`; design/plan: `docs/superpowers/specs/` + `docs/superpowers/plans/2026-07-31-finances-stripe-grade-reporting.md`. |
| `e753a08`…`4a06c26` (Jul 30) | **Thank-You page, rich-text tools, product archive + slideshow control, live CF analytics.** `/thank-you` CMS page + structured shout-out widgets (`ShoutoutCard`, `ShoutoutsEditor`, `src/lib/shoutouts.ts`; migrations `thanks_page_content`, `thanks_shoutouts_and_card_toggle`). `RichTextInput`: **Link button** + **red-glow `flourish` mark** (`.marker-flourish`). About hero headline made CMS-editable; **`RichText` now uses `whitespace-pre-line`** so plain-text line breaks hold (fixed collapsing paragraphs). Product **30-day archive** = `products.archived_at` soft delete + Archived tab + `purge_archived_products()` pg_cron (migration `products_soft_delete_archive`) — fixes the `order_items_product_id_fkey` delete error. Product reorder → **square-card grid** (dnd-kit `rectSortingStrategy`). **Removed shop flip-card auto-spacing** so manual order holds. **Home hero slideshow control**: `products.hero_slideshow` + `hero_order`, per-product + quick toggles + Slideshow tab (migrations `products_hero_slideshow_flag`, `products_hero_order`). `/advertise` **live Cloudflare traffic** via `LiveTrafficStats` → Desktop-built `cloudflare-stats` fn + `app_private_config`; `CF_ANALYTICS_TOKEN` set. ⚠️ Incident: a blind `UPDATE` migration overwrote Opie's live headline edit (recovered from his browser) — **no PITR on free plan; never hard-overwrite editable `site_content` via migration.** |
| `4f016ca` / `183b893` / `c71dda1` | **July 18 follow-on fixes.** `c71dda1`: product **Preview** infinite render-loop (previewProduct re-parsed from localStorage every render → `[product]` effect setState loop; memoized it + `related`) and **style picker now shows in preview** (payload carries `product_group_id`+`styleLabel`, query no longer gated on `!isPreview`). `183b893`: **CMS "Edit Page" panel stale/empty** — `SiteContentProvider` fetched once and never refetched; now refetches on window `focus`/`visibilitychange` (site_content is public-read, no dup rows, so in-tab divergence was impossible → cross-tab staleness). `4f016ca`: **style switcher polish** — stable name-sorted order (selected one only highlights, no float-to-front), bigger buttons, one-word labels (`style_label` set Male/Female/Text/Graphic on the 9 grouped products; fallback strips trailing "Silhouette"). |
| `a3f0bed` | **Product page: Heading Note + rich-text paragraph fix + style switcher.** Migration `20260718000000` adds `products.heading_note` (opt-in HTML block under the title, via `section_visibility.headingNote`, default off) + `products.style_label` (names the on-page style-switcher button, falls back to derived label). Wrapped the live-page `dangerouslySetInnerHTML` renders (description/long_description/bad_advice/heading note) in `.rich-text` so `<p>` breaks stop collapsing (Tailwind reset zeroed the margins). Mobile: grouped products collapse copy behind a toggle; style switches pass `state.styleSwitch` so `ScrollToTop` (App.tsx) + ProductDetail reset effect skip the jump-to-top. Fixed `groupMembers` query key to include the current slug (was serving a stale sibling members cache). Admin: Heading Note block atop COPY card + Style Label input in STYLE GROUP card. Spec: `docs/superpowers/specs/2026-07-18-…`. |
...
| `orders` | Order records — writes via service_role (Stripe webhook). **New (finances build):** `tax_cents`, `stripe_tax_calculation_id`, `stripe_tax_transaction_id` — populated only when `settings.tax_enabled=true`; all 0/null today. |
| `order_items` | Line items per order |
| `custom_requests` | Custom garment inquiry form submissions |
| `settings` | Site config — MUST have row with id=1 for Admin Settings page to work. **New:** `tax_enabled boolean DEFAULT false` — gates all sales-tax collection; DB-controlled, no admin UI toggle. |
| `monthly_snapshots` | Locked monthly P&L snapshots (bookkeeping). **New:** `tax_collected_cents` — set by `close-month`. |
...
| `stripe-webhook` | Handles payment.completed, marks order paid, sends printer + customer emails, awards Pour Points. **v55** wraps post-payment processing in try-catch + logs `[stripe-webhook] PROCESSING ERROR`. **v63** (finances build): if the paid order has a `stripe_tax_calculation_id`, calls `stripe.tax.transactions.createFromCalculation` (records the sale for filing) and persists `orders.tax_cents` + `stripe_tax_transaction_id`. No-op when tax isn't enabled. | STRIPE_SECRET_KEY ✅, STRIPE_WEBHOOK_SIGNING_SECRET ✅, FULFILLMENT_SECRET ✅ |
| `refund-order` | Admin-callable Stripe refund. **v7** (finances build): if the refunded order had a tax transaction, creates a reversal (`stripe.tax.transactions.createReversal`) so refunded tax isn't over-remitted. | STRIPE_SECRET_KEY ✅ |
| `close-month` | Monthly cron — locks a month into `monthly_snapshots`. **v5** (finances build): also sums `orders.tax_cents` for the month into the new `monthly_snapshots.tax_collected_cents`. | — |
...
| `generate-report` | **Extended** (finances build) — admin-JWT or service-role. Adds `format:"json"` (on-screen `{title,period,columns,rows,totals,notes}`) alongside existing `csv`/`html`. 10 report types: pl_statement, order_summary, sales_by_product, expense_detail, stripe_fee_summary, sales_tax, top_customers, refunds_disputes, payout_reconciliation, tax_estimate. | STRIPE_SECRET_KEY ✅ (for refunds_disputes/payout_reconciliation) |
| `calculate-tax` | **NEW** (finances build) — Stripe **Tax Calculation API**. Given `{orderId, address}`, if `settings.tax_enabled` is true: creates a tax calculation, updates the order's PaymentIntent amount, stores `tax_cents` + `stripe_tax_calculation_id` on the order; returns `{taxCents, totalCents}`. No-op (`taxCents:0`) when the flag is off — which is the current live state. | STRIPE_SECRET_KEY ✅ |
| `stripe-balance` | **NEW** (finances build) — returns Stripe `GET /v1/balance` (available/pending) + next scheduled payout, for the Overview KPI tile. Admin/service auth, same pattern as `generate-report`. | STRIPE_SECRET_KEY ✅ |
